package com.lebaillyapp.composeretrouikit.led
 ...
 * # LedBarGraph
 *
 * Composable affichant une barre verticale de LEDs animées simulant un vu-mètre ou indicateur de niveau.
 * Chaque LED peut s’allumer avec un effet de **flicker**, **glow**, et un indicateur de **peak hold** peut
 * rester allumé un certain temps avant de disparaître avec un fondu.
 *
 *  Paramètres personnalisables :
 *
 * @param modifier [Modifier] pour positionner ou styliser le composable.
 * @param numberOfLeds Nombre total de LEDs dans la barre.
 * @param targetPercentage Pourcentage du niveau à atteindre (entre 0f et 1f), utilisé pour allumer les LEDs.
 * @param ledHeight Hauteur d'une LED (Dp).
 * @param ledWidth Largeur d'une LED (Dp).
 * @param spacing Espacement vertical entre chaque LED (Dp).
 * @param ledColors Fonction définissant la couleur de chaque LED selon son index (bas -> haut).
 * @param offColor Couleur des LEDs éteintes.
 * @param alpha Opacité générale des LEDs allumées (0f à 1f).
 * @param glowRadius Rayon du glow lumineux autour des LEDs allumées (en pixels).
 * @param flickerAmplitude Amplitude de scintillement (flicker) des LEDs allumées (0f = pas de variation).
 * @param flickerFrequency Fréquence du flicker (en Hz).
 * @param peakHoldEnabled Active ou non l’affichage temporaire du pic de niveau atteint.
 * @param peakHoldDurationMillis Durée de maintien du niveau de pic (en ms) avant le début du fondu.
 * @param peakHoldFadeDurationMillis Durée du fondu de disparition du pic (en ms).
 *
 *  Effets visuels :
 * - Flicker indépendant par LED avec variation pseudo-aléatoire
 * - Glow réaliste coloré par LED
 * - Barre de peak hold avec fade automatique
 */
@Composable
fun LedBarGraph(
