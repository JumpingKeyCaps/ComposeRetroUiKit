package com.lebaillyapp.composeretrouikit.knob
 ...
 * ### RotaryKnobLimited
 *
 * Un potentiomètre rotatif réaliste avec plage de rotation limitée à une portion d'arc.
 *
 * Ce composant permet de contrôler une valeur discrète entre `minValue` et `maxValue` via un geste de rotation.
 * Il simule un effet physique réaliste avec:
 * - **Effet visuel de profondeur** (fond sombre avec dégradés radiaux et ombres douces)
 * - **Graduation circulaire partielle** (limitée à un arc de 270°, de -135° à +135°)
 * - **Bevel rotatif** (ombre orientée dynamiquement selon la rotation)
 * - **Reflet sur le cœur du bouton**
 * - **Curseur lumineux tournant** (simulant un pointeur)
 * - **Retour haptique** à chaque changement de cran
 * - **Animation de rotation adoucie**
 *
 * Les paramètres permettent une personnalisation fine de l’apparence et du comportement, incluant :
 * tailles relatives, couleurs, styles de graduation, et effet de fond.
 *
 * @param modifier Modificateur pour ajuster la taille et la position du composant
 * @param size Taille totale du knob (par défaut 160.dp)
 * @param steps Nombre total de crans possibles
 * @param minValue Valeur minimale atteignable
 * @param maxValue Valeur maximale atteignable (par défaut = steps - 1)
 * @param initialValue Valeur initiale affichée lors du premier rendu
 * @param onValueChanged Callback appelé à chaque changement de valeur (avec retour haptique)
 * @param tickColor Couleur des repères inactifs
 * @param activeTickColor Couleur du repère actif (valeur actuelle)
 * @param tickLength Longueur des repères (ticks)
 * @param tickWidth Largeur des repères
 * @param tickSpacing Espacement entre le bevel et les repères
 * @param indicatorColor Couleur principale du curseur (pointeur lumineux)
 * @param indicatorSecondaryColor Couleur secondaire du curseur (effet lumineux)
 * @param indicatorLength Longueur du curseur
 * @param indicatorWidth Largeur du curseur
 * @param bevelSizeRatio Ratio (0f–1f) définissant la taille du bevel par rapport au rayon total
 * @param knobSizeRatio Ratio (0f–1f) définissant la taille du cœur (intérieur) par rapport au bevel
 * @param showBackground Active ou désactive le rendu du fond avec effet de profondeur
 * @param backgroundColor Couleur principale du fond
 * @param backgroundShadowColor Couleur utilisée pour les ombres et reflets du fond
 */
@Composable
fun RotaryKnobLimited(
