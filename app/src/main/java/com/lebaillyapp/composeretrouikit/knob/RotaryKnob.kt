 ...
 * ### RotaryKnob
 *
 * Composable réaliste de potentiomètre rotatif inspiré du style analogique (Marshall/Moog),
 * avec effet de lumière dynamique, crans configurables, et retour haptique.
 *
 * Ce composant est entièrement personnalisable et gère :
 * - La rotation continue à 360° avec alignement automatique aux crans.
 * - Un effet de "bevel" tournant (simulant une lumière sur la bordure).
 * - Un cœur avec reflet dynamique et profondeur.
 * - Des repères de graduation colorés, avec distinction du cran actif.
 * - Un curseur lumineux tournant avec le knob.
 * - Une animation fluide de retour au cran.
 * - Un retour haptique à chaque changement de cran.
 *
 * @param modifier Modificateur d’apparence (taille, padding, etc.).
 * @param size Taille du composant (diamètre total du knob).
 * @param steps Nombre total de crans (valeurs discrètes) sur 360°.
 * @param onValueChanged Callback déclenché à chaque changement de cran.
 *
 * @param tickColor Couleur des repères inactifs (ticks).
 * @param activeTickColor Couleur du repère actif (correspondant à la valeur actuelle).
 * @param tickLength Longueur des repères en dehors du bevel.
 * @param tickWidth Épaisseur des repères.
 * @param tickSpacing Espacement entre le bevel et les repères.
 *
 * @param indicatorColor Couleur principale du curseur lumineux.
 * @param indicatorSecondaryColor Couleur secondaire (brillance, dégradé) du curseur.
 * @param indicatorLength Longueur du curseur depuis le bord vers le centre.
 * @param indicatorWidth Épaisseur du curseur.
 *
 * @param bevelSizeRatio Proportion du bevel par rapport au rayon total (0f–1f).
 * @param knobSizeRatio Proportion du cœur par rapport au bevel (0f–1f).
 *
 * @param showBackground Affiche ou non un fond avec effet d'enfoncement.
 * @param backgroundColor Couleur principale du fond.
 * @param backgroundShadowColor Couleur de l'ombre utilisée pour la profondeur du fond.
 *
 */
@Composable
fun RotaryKnob(
