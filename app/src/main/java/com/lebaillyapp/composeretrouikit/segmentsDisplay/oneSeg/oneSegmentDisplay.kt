package com.lebaillyapp.composeretrouikit.segmentsDisplay.oneSeg
 ...
 * ### OneSegmentDisplay
 *
 * Affiche un segment LED réaliste de type 7 segments, avec biseau, effet lumineux (glow),
 * dégradé de lumière (gradient) et animation de scintillement (flicker).
 *
 * Ce composant dessine un segment horizontal stylisé, centré dans un Canvas. Il est conçu
 * pour simuler l’apparence des afficheurs à LED rétro, avec des effets visuels configurables.
 *
 * ##### Caractéristiques :
 * - Forme en trapèze avec biseaux paramétrables.
 * - Effet de glow réaliste autour du segment lorsque `isOn` est `true`.
 * - Dégradé de lumière sur le segment allumé.
 * - Animation de flicker contrôlable (périodique ou aléatoire).
 * - Atténuation automatique du style en mode OFF (segment éteint).
 *
 * @param modifier Modificateur standard Compose pour personnalisation (padding, offset, etc.)
 * @param segmentLengthDp Longueur du segment en Dp (horizontalement).
 * @param segmentWidthDp Largeur du segment en Dp (verticalement).
 * @param bevelSizeDp Taille du biseau sur les coins du segment.
 * @param glowRadiusDp Rayon de l'effet lumineux autour du segment.
 * @param segmentBaseColor Couleur de base du segment (dégradé actif).
 * @param glowColor Couleur du glow autour du segment lorsqu’il est allumé.
 * @param baseGlowAlpha Intensité de base de l’effet de glow (entre 0f et 1f).
 * @param flickerAmplitude Amplitude du flicker (oscillation d’intensité lumineuse).
 * @param flickerFrequencyHz Fréquence du flicker (en Hz) si `flickerRandom` est `false`.
 * @param flickerRandom Active un flicker aléatoire plutôt que périodique.
 * @param isOn Définit si le segment est allumé (`true`) ou éteint (`false`).
 */
@Composable
fun OneSegmentDisplay(
