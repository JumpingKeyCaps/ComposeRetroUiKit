package com.lebaillyapp.composeretrouikit.segmentsDisplay.fourteenSeg
 ...
 *
 * Afficheur LED photoréaliste à 14 segments utilisant Jetpack Compose et Canvas.
 *
 * Ce composant permet d'afficher des chiffres, lettres ou motifs personnalisés à l'aide d'un affichage 14 segments réaliste,
 * avec effets de glow, flicker, biseaux et animation idle intégrée. Chaque segment est dessiné individuellement
 * avec une géométrie précise : segments horizontaux (complets ou demi), verticaux et diagonaux.
 *
 * ## Fonctionnalités
 * - Affichage d'un chiffre (`digit`), d'un caractère (`char`), ou d'une configuration manuelle de segments.
 * - Support des effets visuels personnalisables :
 *   - Glow lumineux (intensité, rayon, couleur)
 *   - Flicker dynamique simulant des variations d’intensité lumineuse
 *   - Biseaux sur tous les segments pour un rendu 3D réaliste
 * - Animation `idle` possible via `rememberInfiniteTransition`
 *
 * ## Ordre des segments
 * L’ordre des 14 segments est le suivant :
 * `A, B, C, D, E, F, G1, G2, H, I, J, K, L, M`
 *
 * ## Priorité d’affichage des segments
 * 1. `manualSegments` si fourni
 * 2. `digit` ou `char` passé directement à la fonction
 * 3. `digit` ou `char` défini dans la `config`
 * 4. `manualSegments` défini dans la `config`
 * 5. Sinon, tous les segments sont éteints
 *
 * @param modifier Le `Modifier` Jetpack Compose pour la personnalisation de la taille et position.
 * @param config Objet `FourteenSegmentConfig` contenant la configuration visuelle des segments (dimensions, couleurs, effets…).
 * @param digit Chiffre optionnel (0–9) à afficher via un mapping standard 14 segments.
 * @param char Caractère ASCII optionnel (lettres majuscules principalement) mappé sur les 14 segments.
 * @param manualSegments Liste booléenne de 14 éléments permettant d’allumer ou éteindre manuellement chaque segment.
 *
 */
@Composable
...
/**
 *  ### FourteenSegmentConfig
 *  easy config file to init 14 segments display
 */
